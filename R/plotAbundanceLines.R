...
plotAbundanceLines <- function(dge, condition, condition_names, plot_type = 'DEG', keep = 0.9, nbarcode = 10, title = '') {
  ...
  df <- stats::aggregate(as.data.frame(counts),list(condition),median)
  ...
  filtered_df <- head(df[order(df[,1],df[,2],decreasing=T),],nrow(df)*keep)
  ...
  if(plot_type=='log2FC'){
    ...
    filtered_df <- head(filtered_df[order(filtered_df$abslog2FC,decreasing = T),],nbarcode)
    filtered_df$abslog2FC <- NULL
    legend_title <- "Highest log2FC barcodes"
  }

  if(plot_type == 'counts'){
    highest_barcodes <- c(rownames(head(df[order(df[,1],decreasing=T),],5)), rownames(head(df[order(df[,2],decreasing=T),],5)))
  ...
  if(plot_type=='DEG'){
    ...
    DEG_counts <- dge$counts[,colnames(data$counts) %in% DEG_samples]
    ...
    DEG.res <- edgeR::exactTest(DEG.obj.final, pair=conditions)
    DEG.barcodes <- rownames(head(DEG.res$table[order(DEG.res$table$PValue,decreasing = F),],nbarcode))
